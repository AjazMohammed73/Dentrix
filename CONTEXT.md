...
1. **`tenants` router** — GET / POST (onboard) / PATCH (status, subscription).
   Super Admin only. MRR/ARR computed in Python over active tenants.
2. **`users` / `staff` router** — GET (global for Super Admin, tenant for Doctor Admin)
   / POST (invite staff) / PATCH (role, status, permissions) / DELETE.
   `assign-doctor-admin` action (promotes a user, grants full permissions, updates the
   tenant's `doctor_admin_*`).
3. **Server-side audit log** — append-only `audit_logs` table; the app DB role gets no
...
4. **Wire the frontend** — replace `context/AuthContext.tsx` and
...
5. **Then** the `PRE_LAUNCH.md` blockers: rate limiting on `/auth/login`, CSV
